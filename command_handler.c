...
* executed command or 0 if no command was executed
*/
...
my_exit(tokens);  /* Handle the "exit" command */
...
tokens[0] = file_path;  /* Update the command with the file path */
...
/* Execute the command using fork and execute function */
free(file_path);
free(tokens);
return (exec_status);
}
